...
# 指定オブジェクトの全てのマテリアルの滑らかさ情報を画像テクスチャにベイクする
def bake_smoothness_texture(arg_object:bpy.types.Object,
  ...
  arg_bakemargin:int=0) -> bpy.types.Image:
    ...
    for material_slot in arg_object.material_slots:
        ...
        manager_material_MRTKstandard.link_inversion_smoothness_roughness_MRTKStandard(arg_material=target_material, arg_uninversion=True)
    ...
    for material_slot in arg_object.material_slots:
        ...
        # 指定マテリアルのアクティブな出力ノードに接続されたMRTKStandardの滑らかさと粗さの反転接続を元に戻す
        manager_material_MRTKstandard.link_inversion_smoothness_roughness_MRTKStandard(arg_material=target_material, arg_uninversion=False)
